...
MISSING_IDENTITY_FAIL_CLOSED negative controls, NO ISIN-prefix heuristics,
snapshot determinism + content sensitivity, and the synthetic fixture runner.
"""
...
def test_exclude_non_crpb():
    for wrong_type in ("SHRS", "OPTN", "FUND", "BOND"):
        d = resolve_disposition(
            _rec(
                instrument_mifir_id=wrong_type,
                issuer_lei="00000000000000000001",
                gleif_legal_jurisdiction_country="ES",
            )
        )
...
def test_exclude_precedence_over_missing_identity():
    # A non-CRPB record with a missing LEI is still EXCLUDE (scope decided first).
    d = resolve_disposition(
        _rec(instrument_mifir_id="FUND", issuer_lei=None, gleif_legal_jurisdiction_country=None)
    )
    assert d.branch is Branch.EXCLUDE
    assert d.reason is Reason.NOT_CRPB
...
def _mixed_table() -> list[SecurityRecord]:
    return [
        ...
        _rec(instrument_mifir_id="SHRS", issuer_lei="00000000000000000002"),
    ]
...
def test_snapshot_sha256_covers_every_branch():
    records = [
        ...
        _rec(instrument_mifir_id="SHRS", issuer_lei="00000000000000000002"),
        _rec(instrument_mifir_id="CRPB", issuer_lei=None),
        _rec(
            instrument_mifir_id="CRPB",
...
def test_fixture_runner_demonstrates_missing_identity_fail_closed():
    ...
    identity_cases = [
        ...
        if r.reason
        in {"MISSING_ISSUER_LEI", "INVALID_ISSUER_LEI", "UNRESOLVABLE_LEGAL_JURISDICTION"}
    ]
    assert identity_cases
    assert all(r.branch == "QUARANTINE" for r in identity_cases)


# --- FIRDS / GLEIF adapters (offline) ----------------------------------------
...
def test_firds_parse_extracts_field5_issuer_lei():
    ...
    assert by_isin["XS0000000101"].instrument_mifir_id == "CRPB"
...
def test_no_network_during_offline_resolution_and_parsing(monkeypatch):
    ...
    records = [
        _rec(
            instrument_mifir_id="CRPB",
            issuer_lei="00000000000000000020",
            gleif_legal_jurisdiction_country="ES",
        )
    ]
